...
def _ps_acme_score(ph, data):
    ...
    p = 10000000 * pfun #AC, before it was 1000 * pfun
...
def manual_ps(data, notebook=False):
    ...
    if notebook:
        ...
        plt.title('Absorption spectrum')#AC
        interactive, = plt.plot(data.real, lw=1, color='black')

        axcolor = 'white'
        axpc0 = plt.axes([0.25, 0.05, 0.65, 0.03], facecolor=axcolor)#AC, adjusted heigth
        axpc1 = plt.axes([0.25, 0.10, 0.65, 0.03], facecolor=axcolor)#AC, adjusted heigth
        axpiv = plt.axes([0.25, 0.15, 0.65, 0.03], facecolor=axcolor)#AC, adjusted heigth
        axpst = plt.axes([0.25, 0.20, 0.15, 0.04], facecolor=axcolor)#AC, adjusted heigth

        spc0 = Slider(axpc0, 'p0', -360, 360, valinit=0)
        spc1 = Slider(axpc1, 'p1', -360*15, 360*15, valinit=0,valstep=10)#AC, enhanced maximum values for phasing
        ...
        plt.show(block=False)#AC, True freezes all the script
        plt.figure()#AC, create a dummy figure that, when clicked continue the program
        plt.text(0, 0.9, 'INSTRUCTION:', bbox=dict(facecolor='red', alpha=0.5))#AC
        plt.text(0, 0.75, '1)Reduce this windows to an icon;', bbox=dict(facecolor='red', alpha=0.5))#AC
        plt.text(0, 0.5, '2)Select a phase in the other windows;', bbox=dict(facecolor='red', alpha=0.5))#AC
        plt.text(0, 0.40, 'WARNING! The returned phase is the last one on the other window.', bbox=dict(facecolor='red', alpha=0.5))#AC
        plt.text(0, 0.25, '3)Re-open and click anywhare on this window AFTER the phase has been selected!', bbox=dict(facecolor='red', alpha=0.5))#AC
        figManager = plt.get_current_fig_manager()#AC
        figManager.window.showMaximized()#AC
        plt.show()#AC
        plt.waitforbuttonpress()#AC
        plt.close('all')#AC
        p0 = spc0.val#-spc1.val*spiv.val/data.size #AC,commented part
        p1 = spc1.val
        pivot = spiv.val #AC
        return p0, p1, pivot #AC, added pivot to returned values
